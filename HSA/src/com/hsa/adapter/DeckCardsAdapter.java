...
public class DeckCardsAdapter extends BaseAdapter{
	
	private final List<GraphicalAggregation> grapichalsAggregations;
	private LayoutInflater inflater;
	private Context context;
 
	public DeckCardsAdapter(Context context, List<GraphicalAggregation> ga) {
...
//	}
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
    	View retval = LayoutInflater.from(parent.getContext()).inflate(R.layout.h_list_view_item, null);
        TextView title = (TextView) retval.findViewById(R.id.title);
        title.setText("3");
        ImageView img = (ImageView) retval.findViewById(R.id.image);
        img.setImageResource(grapichalsAggregations.get(position).getImage());
